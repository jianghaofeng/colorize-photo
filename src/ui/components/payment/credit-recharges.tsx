...
export function CreditRecharges({
    className = "",
}: CreditRechargesProps) {
    console.log("[DEBUG] Rendering CreditRecharges component");
    const t = useTranslations("Payment");
    const { error, hasMore, isLoading, loadMore, recharges } = useCreditRecharges(10);
    
    console.log("[DEBUG] CreditRecharges hook state:", { error, isLoading, rechargesCount: recharges.length });
    ...
    if (error) {
        console.log("[DEBUG] Rendering error state in CreditRecharges, error:", error);
        return (
            <div className={`
              space-y-4
              ${className}
