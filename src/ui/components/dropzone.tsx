...
export const formatBytes = (
  bytes: number,
  decimals = 2,
  size?: "bytes" | "EB" | "GB" | "KB" | "MB" | "PB" | "TB" | "YB" | "ZB",
) => {
  const k = 1000;
    ...
    <div className={cn("flex flex-col", className)}>
      {files.map((file, idx) => {
        ...
        return (
          <div
            className={`
              flex items-center gap-x-4 border-b py-2
              first:mt-4
              last:mb-4
            `}
            key={`${file.name}-${idx}`}
          >
            ...
            {!loading && !isSuccessfullyUploaded && (
              <Button
                className={`
                  shrink-0 justify-self-end text-muted-foreground
...
const DropzoneEmptyState = ({ className }: { className?: string }) => {
  const { inputRef, isSuccess, maxFiles, maxFileSize } = useDropzoneContext();
    ...
    <div className={cn("flex flex-col items-center gap-y-2", className)}>
      ...
      <div className="flex flex-col items-center gap-y-1">
        <p className="text-xs text-muted-foreground">
          ...
          <a
            className={`
              cursor-pointer underline transition
              hover:text-foreground
            `}
